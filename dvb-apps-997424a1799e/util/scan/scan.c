			      ...
			      int descriptors_loop_len, void *data)
{
	while (descriptors_loop_len > 0) {
		...
		case 0x0a:
			...
			//info("parse_iso639_language_descriptor\n");
			break;
		...
		case 0x43:
			...
			//info("parse_satellite_delivery_system_descriptor\n");
			break;
		...
		case 0x53:
			...
			//info("parse_ca_identifier_descriptor\n");
			break;
		...
		case 0x62:
			...
			//info("parse_frequency_list_descriptor\n");
			break;
...
static int read_sections (struct section_buf *s)
{
	...
	/*FILE *fd = fopen("/root/save.ts", "wb");
    	if (fd == NULL) 
    	{
        	errorn("open save.ts err!! \n");
        	return -1;
    	}*/
	...
	/*if(fwrite(s->buf, (section_length+3), 1, fd))
	{
		info("fwrite ok!\n");
		fclose(fd);
	}*/
...
static void read_filters (void)
{
	...
	for (i = 0; i < n_running; i++) {
		...
		if (poll_fds[i].revents)
		{
			...
			//info("n_running=%d,revents-done=%d\n",i,done);
		}
		else
		{
			done = 0; /* timeout */
			//info("n_running=%d,done=%d\n",i,done);
		}
