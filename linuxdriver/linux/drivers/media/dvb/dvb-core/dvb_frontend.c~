...
static int dvb_frontend_thread(void *data)
{
	...
	while (1) {
		...
		if (fepriv->reinitialise) {
			dvb_frontend_init(fe);
			if (fepriv->standard != -1){
				fe->ops.set_standard(fe, (u16 *)&fepriv->standard);
			}
			if (fepriv->tone != -1) {
			...
			unsigned int cmd, void *parg)
{
	...
	case FE_SET_STANDARD:
		printk ("FE_SET_STANDARD.\n");
		if(fe->ops.set_standard)
			err = fe->ops.set_standard(fe, (u16*)parg);
			fepriv->standard = *(u16 *)parg;
		break;
