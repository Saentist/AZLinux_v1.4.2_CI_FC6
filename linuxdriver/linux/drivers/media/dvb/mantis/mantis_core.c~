...
int mantis_core_init(struct mantis_pci *mantis)
{
	...
	if ((err = mantis_dvb_init(mantis)) < 0) {
		...
		return err;dprintk(verbose, MANTIS_ERROR, 1, "err= 0x%x", err);
	}
	...
	u32 reg;
        ...
        switch(bitpos)
        {
            case 12:
                if(value == 0){
                    	reg &= 0xffffefff;
			mantis->A121314Status  &= 0x06;		
                }
                else{
                     	reg |= (u32)(1 << 12);
			mantis->A121314Status  |= 0x01;
                }
		  break;
            ...
            case 13:
                if(value == 0){
                    	reg &= 0xffffdfff;
			mantis->A121314Status  &= 0x05;
                }
                else{
                    	reg |= (u32)(1 << 13);
			mantis->A121314Status  |= 0x02;
                }
		  break;
            ...
            case 14:
                if(value == 0){
                     	reg &= 0xffffbfff;
			mantis->A121314Status  &= 0x03;
                }
                else{					
                	reg |= (u32)(1 << 14);
		     	mantis->A121314Status  |= 0x04 ;
                }
		  break;
	    default:
	        dprintk(verbose, MANTIS_ERROR, 1, "wrong position.");
        }
	
        mmwrite(reg, MANTIS_GPIF_ADDR);
...
//direction  = 0 ,not pass CI ; 1 : pass to CI
