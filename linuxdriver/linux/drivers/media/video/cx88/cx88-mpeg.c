			    ...
			    struct cx88_buffer   *buf)
{
	...
	cx_set(MO_PCI_INTMSK, core->pci_irqmask | MO_INT_TSINT);
...
static void cx8802_mpeg_irq(struct cx8802_dev *dev)
{
	...
	if (status & 0x01) {
		...
		dprintk( 0, "cx8802_mpeg_irq---->cx88_wakeup\n" );//add by may
...
static irqreturn_t cx8802_irq(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	if (MAX_IRQ_LOOP == loop) {
		...
		printk(KERN_WARNING "%s/0: irq loop -- clearing mask[cx8802_irq]\n",
		       core->name);
...
int cx8802_init_common(struct cx8802_dev *dev)
{
	...
	if (!core->initialized) {
		...
		cx88_reset(core);
		cx88_i2c_init(core, dev->pci);
		cx88_call_i2c_clients (core, TUNER_SET_STANDBY, NULL);
		cx88_card_setup(core);
		cx88_ir_init(core, dev->pci);
		core->initialized = 1;
	}
