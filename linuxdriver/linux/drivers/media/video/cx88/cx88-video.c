...
static irqreturn_t cx8800_irq(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	core->int_status = cx_read(MO_PCI_INTSTAT);
	for (loop = 0; loop < 10; loop++) {
		status = cx_read(MO_PCI_INTSTAT) &
				(core->pci_irqmask | MO_INT_VIDINT
						   | MO_INT_I2CDONE);

		if (0 == status)
			goto out;
		cx_write(MO_PCI_INTSTAT, core->int_status);
		handled = 1;

		if (core->int_status & core->pci_irqmask)
			cx88_core_irq(core, core->int_status);
		if (core->int_status & 0x01)
			cx8800_vid_irq(dev);
	}
 out:
				    ...
				    const struct pci_device_id *pci_id)
{
	...
	/*	I2C Initialization	*/
	if (!core->initialized) {
		init_waitqueue_head(&core->i2c_wq);
		cx88_reset(core);
		cx88_i2c_init(core, pci_dev);
		cx88_call_i2c_clients (core, TUNER_SET_STANDBY, NULL);
		cx88_card_setup(core);
		cx88_ir_init(core,pci_dev);
		core->initialized = 1;
	}
